...
struct node // constract node
{
    ...
    double Medicine[100] = {2.00, 3.00, 1.00, 4.00, 1.00, 5.00, 7.00, 4.00, 3.00, 5.00,12.00, 13.00, 11.00, 14.00, 11.00, 15.00, 17.00, 14.00, 13.00, 15.00};
    double total;
...
int main()
{
    ...
    do
    {
        ...
        case 6:
            cout << "\nHow many medicines to insert? :\n";
            cin >> count;
            for (int i = 0; i < count; i++)
            {
                cout << "Medicine "<<i<< "Information:\n";
                medicine.read_data();
                medicine.pack();
                medicine.write_to_file();
            }
            system("PAUSE");
            break;
        case 7:
...
void medicineType::take_order()
{
    ...
    temp = new node;
                ...
                for(int j=1;j<=10;j++){
                   cout<<j-1<<"\t"<<"\tOTC"<<"\t\t"<<"     "<<temp->medicineName[j]<<"      \t\t"<<temp->Medicine[j-1]<<endl;
...
void medicineType::pack()
{
    string temp;
    buffer.erase();
    buffer += d_ID + "|" + d_TYPE + "|" + d_NAME + "|" + to_string(d_PRICE) + "$" + "\n";
}
void medicineType::write_to_file()
{
    int pos;
    fstream file;
    file.open("Add_stock.txt", ios::out | ios::app);
    pos = file.tellp();
    file << buffer;
    file.close();
...
void medicineType::disp()
{
    ...
    cout << "\n";
    system("cat Add_stock.txt");
}
void medicineType::sort_index()
{
...
int medicineType::search_index(string key)
{
...
void medicineType::search(string key)
{
    int pos = 0, t;
    string buffer;
    buffer.erase();
...
void medicineType::read_from_file(int pos)
{
    int address, i;
    fstream file;
    file.open("Add_stock.txt", ios::in | ios::app);
    address = Address_list[pos];
    file.seekp(address, ios::beg);
    buffer.erase();
    getline(file, buffer);
    cout << "\nFound the medicine: " << buffer;
    file.close();
}
void medicineType::remove(string key)
{
    int pos = 0, t, choice;
    string
        buffer;
    ...
    if (pos == -1)
        cout << endl
             << "Not possible to remove the Medicine";
    else if (pos >= 0)
    {
        read_from_file(pos);
        cout << "\nDelete?(1/0):";
...
void medicineType::delete_from_file(int pos)
{
    int i, address;
    fstream file;
    file.open("Add_stock.txt");
    address = Address_list[pos];
    file.seekp(address, ios::beg);
    file.put('*');
    cout << "\nMedicine Information Deleted : ";
    for (i = pos; i < count; i++)
    {
